...
# this the name and folder of your bucket
...
def get_gcs_blob_name(file_name: str) -> str:
...
def upload_checkpoint(
    ...
    try:
        ...
        for layer_no in target_layers:
            file_name = get_x_filename(model_name, layer_no, pool_type, split)
            local_path = get_local_path(file_name)
            blob = BUCKET.blob(get_gcs_blob_name(file_name))
            blob.upload_from_filename(local_path)
        
        # upload the target y values
        y_file_name = get_y_filename(model_name, pool_type, split)
        local_path = get_local_path(y_file_name)
        blob = BUCKET.blob(get_gcs_blob_name(y_file_name))
        blob.upload_from_filename(local_path)
        ...
        chkpt_blob = BUCKET.blob(get_gcs_blob_name(chkpt_file_name))
        ...
        torch.save(rng_state, get_local_path(rng_file_name))
        rng_blob = BUCKET.blob(get_gcs_blob_name(rng_file_name))
        rng_blob.upload_from_filename(get_local_path(rng_file_name))

        print("Checkpoint uploaded successfully.")
    except Exception as e:
        print(f"Error uploading to GCS: {e}")

def resume_checkpoint_helper(split: DataSplit, target_layers: List[int], pool_type: PoolingStrategy, model_name: ModelName):
    try:
        ...
        rng_file_name = get_rng_state_filename(model_name, pool_type, split)
        local_path = get_local_path(rng_file_name)
        blob = BUCKET.blob(get_gcs_blob_name(rng_file_name))
        if blob.exists() and not os.path.exists(local_path):
            print(f"Downloading {blob.name} to {local_path}...")
            blob.download_to_filename(local_path)
...
def resume_checkpoint_if_exists(
        ...
        model_name: ModelName, 
        n: Optional[int] = None, 
        d: Optional[int] = None) -> Tuple[int, dict[int, np.memmap], np.memmap, Optional[Dict]]:
    checkpoint_idx: int = resume_checkpoint_helper(split=split, target_layers=target_layers, pool_type=pool_type, model_name=model_name)
    ...
    x_arr_dict: dict[int, np.memmap] = {
        layer_no: open_memmap(
               get_local_path(get_x_filename(model_name, layer_no, pool_type, split)),
    ...
    y_arr: np.memmap = open_memmap(get_local_path(get_y_filename(model_name, pool_type, split)),       mode=mode, dtype="int64", shape=(n,) if mode == "w+" else None)
    
    # load the RNG sequence state if it was downloaded
    rng_state = None
    rng_local_path = get_local_path(get_rng_state_filename(model_name, pool_type, split))
